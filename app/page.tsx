...
const drilldowns: Record<string, { eyebrow:string; title:string; value:string; summary:string; rows:[string,string][]; assumption:string; action:string; source:string }> = {
  ...
  "billing-utilisation": { eyebrow:"Operating guardrail 01", title:"FLA billing utilisation", value:">88%", summary:"This internal FY26/27 threshold protects the revenue capacity and gross-profit economics assumed for the FLA delivery base.", rows:[["Source threshold",">88%"],["Calculation","Billed days ÷ available days"],["Management cadence","Weekly by team and client"],["Accountable owner","COO / Delivery leads"]], assumption:"The threshold comes from Section 11 of the executive analysis; it is a management target, not an external benchmark.", action:"Review utilisation weekly, rebalance work between teams, accelerate client allocation and hold discretionary hiring where unassigned capacity persists.", source:"Executive analysis · Section 11 operational KPIs" },
  "client-retention": { eyebrow:"Operating guardrail 02", title:"Client retention", value:">92%", summary:"The plan depends on protecting the recurring client base that supports managed-service revenue, referrals and AI Digital cross-sell.", rows:[["Source threshold",">92%"],["Calculation","Retained clients ÷ opening client base"],["Management cadence","Monthly cohort review"],["Accountable owner","CCO / Entity leaders"]], assumption:"The 92% level is the FY26/27 internal Board target in Section 11, equivalent to keeping annual client churn below 8%.", action:"Maintain named renewal plans, flag accounts 120 days before renewal, run executive sponsor reviews for at-risk clients and track churn reasons to closure.", source:"Executive analysis · Section 11 operational KPIs" },
  ...
  "pipeline": { eyebrow:"Operating guardrail 04", title:"Qualified pipeline coverage", value:"1.5×", summary:"The 1.5× target provides a conversion buffer between qualified opportunities and the quarterly revenue target, particularly for the UK plan.", rows:[["Current UK coverage","~1.25×"],["Required coverage","1.5×"],["Existing / expansion","$1.72M committed"],["Active proposals","$1.43M high confidence"]], assumption:"Section 08 requires 1.5× coverage before Board approval of the UK budget; Section 11 adopts it as the group operating KPI.", action:"Review named opportunities weekly, enforce stage-exit criteria, add pipeline where weighted coverage falls below 1.5× and show movement in the monthly Board pack.", source:"Executive analysis · Sections 08 and 11" },
  "win-rate": { eyebrow:"Operating guardrail 05", title:"Proposal win rate", value:">40%", summary:"The revenue plan assumes qualified pipeline converts efficiently; a lower rate would require materially more pipeline and selling capacity.", rows:[["Source threshold",">40%"],["Calculation","Closed-won ÷ proposals submitted"],["Management cadence","Monthly by market and service"],["Accountable owner","CCO / Sales leads"]], assumption:"The threshold is the FY26/27 internal target in Section 11 rather than a claimed market benchmark.", action:"Run win-loss reviews, tighten proposal qualification, standardise commercial approvals and coach teams by market where conversion falls below target.", source:"Executive analysis · Section 11 operational KPIs" },
  "fla-turnover": { eyebrow:"Operating guardrail 06", title:"FLA employee turnover", value:"<18%", summary:"The FLA plan scales from 620 to 761 people; uncontrolled attrition would consume recruitment capacity and put client delivery at risk.", rows:[["Source threshold","<18% annualised"],["Calculation","FLA exits ÷ average headcount × 12"],["Management cadence","Monthly by team and tenure"],["Accountable owner","CPO / COO"]], assumption:"The 18% ceiling is the FY26/27 internal target in Section 11, aligned to protecting the headcount ramp in the operating case.", action:"Use manager-level attrition heatmaps, stay interviews, compensation and workload reviews, and 30/60/90-day retention checks for priority roles.", source:"Executive analysis · Sections 11 and 13" },
  "time-to-recruit": { eyebrow:"Operating guardrail 07", title:"FLA time to recruit", value:"<28 days", summary:"Revenue capacity depends on filling approved FLA roles quickly enough to keep the 761-person delivery plan on schedule.", rows:[["Source threshold","<28 days"],["Calculation","Job opening to accepted offer"],["Management cadence","Weekly recruitment funnel"],["Accountable owner","CPO / Recruitment lead"]], assumption:"The 28-day target is the FY26/27 internal benchmark in Section 11 and supports the headcount ramp identified as a key execution lever.", action:"Pre-build specialist talent pools, set interview SLAs, monitor ageing by funnel stage and escalate roles that exceed 21 days before the target is missed.", source:"Executive analysis · Sections 02, 11 and 13" },
  "ai-active-customers": { eyebrow:"Operating guardrail 08", title:"AI Digital active customers", value:"69", summary:"This is the FY26/27 go-live operating target: customers actively using the platform, not simply clients counted in the broader adoption scenario.", rows:[["Active-customer target","69 live customers"],["Expected adoption scenario","~123 converted clients"],["Revenue per active client","$30.9k average ARR"],["Management cadence","Monthly funnel and activation review"]], assumption:"Section 11 defines 69 active customers; Section 09 models ~123 phased conversions at 30% adoption. The Board should monitor both measures separately.", action:"Track eligible, contacted, converted, implementation and live customers as separate funnel stages; use a six-month gate and pause incremental opex if performance is below the bear case.", source:"Executive analysis · Sections 09 and 11" },
};
