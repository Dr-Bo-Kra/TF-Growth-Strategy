...
const drilldowns: Record<string, { eyebrow:string; title:string; value:string; summary:string; rows:[string,string][]; assumption:string; action:string; source:string }> = {
  "target-revenue": { eyebrow:"Group trajectory", title:"FY28/29 group revenue", value:"$72.830M", summary:"The Excel-controlled full-group model more than doubles the FY25/26 base.", rows:[["FA / FLA","$34.627M"],["TF core geographies","$28.373M"],["Embedded digital lines","$6.061M"],["QGCC","$0.360M"],["AI Digital · TF channel","$0.512M"],["AI Digital · FA channel","$1.844M"],["AI Digital · Backroom channel","$1.053M"],["Full-group total","$72.830M"]], assumption:"Backroom's AI contribution is included inside AI Digital and is not added again as a separate acquisition uplift.", action:"Review entity performance quarterly against the bridge, not only the consolidated total.", source:"TF+FA Consolidated row 21 + Tech Consolidated rows 12–19" },
  "target-margin": { eyebrow:"Profitability", title:"FY28/29 EBITDA margin", value:"27.8%", summary:"The Excel-controlled margin expands through FA operating leverage, AI contribution and the recurring synergy register.", rows:[["FY25/26","9.1%"],["FY26/27","14.3%"],["FY27/28","26.4%"],["FY28/29","27.8%"]], assumption:"Includes $1.650M annual post-integration synergies from FY26/27 onward.", action:"Track pre- and post-synergy EBITDA separately so delivery slippage remains visible.", source:"TF+FA Consolidated rows 123-126 + Tech Consolidated rows 46-47" },
  "revenue-25": { eyebrow:"FY2025/26 base", title:"Group revenue", value:"$34.054M", summary:"The opening year is the Excel-controlled reference base for the four-year growth case.", rows:[["FA / FLA","$23.738M"],["TF core geographies","$9.580M"],["QGCC","$0.719M"],["Other no-margin revenue","$0.017M"],["AI Digital","—"],["Group EBITDA","$3.114M"]], assumption:"No AI Digital or recurring integration synergy is included in the base year.", action:"Use this base consistently when calculating growth and margin expansion.", source:"TF+FA Consolidated row 21 and row 123" },
  "revenue-27": { eyebrow:"FY2027/28", title:"Group revenue", value:"$59.044M", summary:"The third year reflects scaled FA delivery, stronger TF geographic contribution and a larger digital revenue base.", rows:[["FA / FLA","$29.902M"],["TF core geographies","$20.169M"],["Embedded digital lines","$5.471M"],["QGCC","$0.360M"],["AI Digital · TF channel","$0.454M"],["AI Digital · FA channel","$1.635M"],["AI Digital · Backroom channel","$1.053M"],["Full-group total","$59.044M"]], assumption:"Backroom is included within the separate AI Digital total, not as an additional operating-company revenue line.", action:"Reconcile entity delivery and digital contribution to the full-group total each quarter.", source:"TF+FA Consolidated row 21 + Tech Consolidated rows 12–19" },
  "ebitda-25": { eyebrow:"FY2025/26 base", title:"Group EBITDA", value:"$3.114M", summary:"The Excel-controlled base year establishes the starting profitability of the combined operating case.", rows:[["Group EBITDA","$3.114M"],["EBITDA margin","9.1%"],["Annual synergies","—"],["Group revenue","$34.054M"]], assumption:"No recurring integration synergy or AI Digital contribution is included in the base year.", action:"Keep the base-year result unchanged when assessing future operating leverage.", source:"TF+FA Consolidated row 21 and row 123" },
  "ebitda-27": { eyebrow:"FY2027/28", title:"Full-group post-synergy EBITDA", value:"$15.612M", summary:"Profitability accelerates as FA operating leverage, AI contribution and recurring synergies compound.", rows:[["TF + FA pre-synergy EBITDA","$12.636M"],["Annual synergies","$1.650M"],["TF + FA post-synergy EBITDA","$14.286M"],["AI Digital EBITDA","$1.326M"],["Full-group EBITDA","$15.612M"],["EBITDA margin","26.4%"]], assumption:"AI Digital is added separately to the TF+FA post-synergy result.", action:"Separate underlying EBITDA, realised synergies and AI Digital in Board reporting.", source:"TF+FA Consolidated rows 123-126 + Tech Consolidated row 46" },
  "ebitda-28": { eyebrow:"FY2028/29", title:"Full-group post-synergy EBITDA", value:"$20.275M", summary:"The final forecast year captures the mature operating leverage of the Excel-controlled combined group.", rows:[["TF + FA pre-synergy EBITDA","$17.139M"],["Annual synergies","$1.650M"],["TF + FA post-synergy EBITDA","$18.789M"],["AI Digital EBITDA","$1.486M"],["Full-group EBITDA","$20.275M"],["EBITDA margin","27.8%"]], assumption:"AI Digital is added separately to the TF+FA post-synergy result.", action:"Validate that margin growth is supported by entity performance, not only consolidation effects.", source:"TF+FA Consolidated rows 123-126 + Tech Consolidated row 46" },
  ...
  "client-base": { eyebrow:"Distribution", title:"Combined client base", value:"410", summary:"The existing relationships form the distribution advantage for AI Digital, but revenue is recognised as each segment activates.", rows:[["TF clients","50 · 15 target conversions"],["FA clients","180 · 55 target conversions"],["Backroom clients","180 · 54 target conversions"],["Expected year-end active","124 customers"]], assumption:"The 30% adoption assumption produces 124 year-end active customers; the Excel model begins Backroom revenue in February 2027.", action:"Report eligible, pitched, signed, implemented and live customers separately each month.", source:"Tech Consolidated · rows 12–14 and monthly columns" },
  "revenue-26": { eyebrow:"FY2026/27", title:"Group revenue", value:"$42.359M", summary:"The Excel-controlled case grows 24.4% year on year, led by TF expansion, FA scale and the first year of AI Digital.", rows:[["FA / FLA","$25.672M"],["TF core geographies","$14.193M"],["QGCC","$0.360M"],["AI Digital · TF channel","$0.336M"],["AI Digital · FA channel","$1.211M"],["AI Digital · Backroom channel","$0.439M"],["AI Digital · QGCC channel","$0.148M"],["Full-group total","$42.359M"]], assumption:"Backroom begins contributing AI Digital revenue in February 2027 and is already included in the $2.134M AI Digital total.", action:"Use the $42.359M Excel-controlled case as the Board baseline.", source:"TF+FA Consolidated row 21 + Tech Consolidated rows 12–19" },
  "gp-26": { eyebrow:"FY2026/27", title:"Group gross profit", value:"$16.360M", summary:"The Excel-controlled group mix produces a 38.6% gross margin, with AI Digital providing the highest unit margin.", rows:[["Group GP margin","38.6%"],["FA GP margin","30.2%"],["TF standalone GP margin","50.2%"],["AI GP margin","54.6%"]], assumption:"Full-group gross profit equals $15.195M TF+FA plus $1.165M AI Digital.", action:"Monitor GP per FTE and pricing uplift alongside the headline margin.", source:"TF+FA Consolidated row 41 + Tech Consolidated row 31" },
  "ebitda-26": { eyebrow:"FY2026/27", title:"Full-group post-synergy EBITDA", value:"$6.075M", summary:"The full-group result combines TF+FA pre-synergy EBITDA, the annual synergy register and separate AI Digital EBITDA, producing a 14.3% margin.", rows:[["TF + FA pre-synergy EBITDA","$3.835M"],["Annual synergy register","$1.650M"],["TF + FA post-synergy EBITDA","$5.485M"],["AI Digital EBITDA","$0.590M"],["Full-group EBITDA","$6.075M"],["Group margin","14.3%"]], assumption:"AI Digital is separate from the $5.485M TF+FA post-synergy result.", action:"Report TF+FA, synergies and AI Digital as separate bridge lines every month.", source:"TF+FA Consolidated rows 123-126 + Tech Consolidated row 46" },
  "fa-engine": { eyebrow:"Scale Engine", title:"FA / FLA FY26/27 revenue", value:"$25.7M", summary:"Frontline Accounting is the Scale Engine and the largest contributor to consolidated revenue.", rows:[["FY25/26 revenue","$23.7M"],["FY26/27 revenue","$25.7M"],["FY28/29 revenue","$34.6M"],["FY28/29 EBITDA margin","28.4%"]], assumption:"Headcount reaches 761 in FY26/27 and facilities commitments remain staged.", action:"Alert the Board at 5% and 10% headcount misses; tie leases to 80% attainment.", source:"FA Budget Forecast · FA Budget annual columns" },
  "tf-engine": { eyebrow:"Growth Formula", title:"Talent Formula FY26/27 revenue", value:"$14.193M", summary:"Talent Formula is the Growth Formula, combining recruitment capability with geographic expansion.", rows:[["FY25/26 core revenue","$9.597M"],["FY26/27 core revenue","$14.193M"],["FY28/29 core revenue","$29.342M"],["Four-year CAGR","45.2%"]], assumption:"Core revenue includes TF's embedded digital line but excludes QGCC and FA/FLA revenue so the growth engine is shown consistently.", action:"Appoint the UK Growth Lead and reach 1.5× qualified coverage before Q2 lock.", source:"TF+FA Consolidated · annual revenue rows 9–13" },
  "ai-engine": { eyebrow:"Margin Magic", title:"AI Digital FY26/27 revenue", value:"$2.134M", summary:"AI Digital is Margin Magic: recurring platform economics distributed through the existing client base.", rows:[["TF channel","$0.336M"],["FA channel","$1.211M"],["Backroom channel","$0.439M"],["QGCC channel","$0.148M"],["Gross profit","$1.165M"],["EBITDA","$0.590M"]], assumption:"TF and FA revenue starts in September 2026; the Excel model begins Backroom revenue in February 2027.", action:"Gate incremental opex against the monthly conversion funnel and track channel conversion separately.", source:"Tech Consolidated · rows 12–19, 31 and 46" },
  ...
  "ai-active-customers": { eyebrow:"Budget assumption · verified", title:"AI Digital active customers", value:"124", summary:"The 30% adoption target includes 55 FA, 15 TF and 54 Backroom customers, with Backroom revenue beginning in February 2027.", rows:[["FA + TF","55 + 15 customers"],["Backroom","54 customers · February start"],["Total addressable","410 clients"],["Adoption target","30%"]], assumption:"The customer count is budget-verified, but adoption remains unvalidated without pilot data or signed LOIs.", action:"Require evidence from 20-30 warm conversations and report pitched, demo, trial, signed and live customers monthly.", source:"Tech Consolidated · customer rows 12–14" },
  "ai-mrr": { eyebrow:"Direct from budget · verified", title:"AI Digital June run-rate MRR", value:"$242,483", summary:"The June run-rate comes directly from the budget worksheet after TF and FA activate in September and Backroom begins contributing in February.", rows:[["Sep 26","$179,338"],["Feb-Jun 27","$242,483"],["Platform","$2,700 · all adopters"],["AI employee","$1,800 · 30% of adopters"]], assumption:"The budget uses segment-specific internally derived realised rates; the $15,000 setup fee applies to 30% of adopters and is excluded from MRR.", action:"Track actual MRR against the monthly build; treat September below $150k as the earliest conversion warning.", source:"Tech Consolidated · monthly revenue rows 12–19" },
};
...
const strategyThemes = [
  { keywords:["dashboard","dashboard comparison","dashboard says","dashboard numbers","reconcile dashboard","40.2","69.4","42.4","72.8","scope"], answer:"The figures reconcile by scope. TF+FA Consolidated reports $40.225M in FY26/27 and $69.421M in FY28/29. Adding the separate Tech Consolidated AI Digital model produces full-group revenue of $42.359M and $72.830M. These Excel-controlled totals are the Board-page baseline; older rounded executive-analysis figures are no longer used.", sources:["TF+FA Consolidated · row 21", "Tech Consolidated · row 19"] },
...
function answerStrategy(question:string) {
  ...
  if (asksRevenueChange && (/\b27\b/.test(query) || /26\s*[\/–-]\s*27/.test(query)) && (/\b28\b/.test(query) || /27\s*[\/–-]\s*28/.test(query))) {
    return { text:"Full-group revenue increases from $34.054M to $42.359M in FY26/27, a 24.4% year-on-year increase. It then rises to $59.044M in FY27/28, a 39.4% increase. These figures come from TF+FA Consolidated plus the separate Tech Consolidated AI Digital model.", sources:["TF+FA Consolidated · row 21", "Tech Consolidated · row 19"] };
  }
...
export default function Home() {
  ...
  const currentDetail = selected === "ai-scenario" ? {
    ...
    assumption:"Eligible clients enter the funnel in phases: TF and FA from September, with Backroom revenue beginning in February 2027 in the Excel model.", action:"Compare actual conversions, live customers, MRR and revenue per customer with this case every month.", source:"Tech Consolidated · monthly revenue rows 12–19"
  ...
  return <main className={`section-${active}`}>
    ...
    <section id="thesis" className="chapter opening">
      ...
      <div className="north-star">
        ...
        <Drill id="target-revenue" className="hero-drill">$72.8M</Drill><p>Group revenue</p>
        <div className="ns-rule"><i/><span>2.14× FY25/26 base</span></div>
        <div className="margin"><span>EBITDA margin</span><Drill id="target-margin">27.8%</Drill></div>
      </div>
    ...
    <section id="trajectory" className="chapter trajectory-section">
      ...
      <div className="section-intro"><h2>Growth earns its value<br/>through <em>operating leverage.</em></h2><p>The Excel-controlled case moves from $34.054M to $72.830M in revenue while full-group EBITDA expands from $3.114M to $20.275M.</p></div>
      <div className="trajectory-grid">
        <div className="financial-chart">
          {[{y:"FY25/26",r:34.054,e:3.114,rid:"revenue-25",eid:"ebitda-25"},{y:"FY26/27",r:42.359,e:6.075,rid:"revenue-26",eid:"ebitda-26"},{y:"FY27/28",r:59.044,e:15.612,rid:"revenue-27",eid:"ebitda-27"},{y:"FY28/29",r:72.830,e:20.275,rid:"target-revenue",eid:"ebitda-28"}].map((d,i)=><div className="year" key={d.y}><div className="bars"><button type="button" aria-label={`Explore ${d.y} revenue`} onClick={()=>setSelected(d.rid)} className="revenue" style={{height:`${d.r/72.83*100}%`}}><span>{money(d.r)}</span></button><button type="button" aria-label={`Explore ${d.y} EBITDA`} onClick={()=>setSelected(d.eid)} className="ebitda" style={{height:`${Math.max(d.e/20.275*74,6)}%`}}><span>{money(d.e)}</span></button></div><b>{d.y}</b><small>{["9.1%","14.3%","26.4%","27.8%"][i]} margin</small></div>)}
        ...
        <div className="math-stack">
          <article><small>Revenue CAGR</small><strong>28.8%</strong><p>FY25/26 → FY28/29</p></article>
          <article className="dark"><small>Margin expansion</small><strong>+18.7pp</strong><p>Real operating leverage</p></article>
          <article><small>FY26/27 gross profit</small><Drill id="gp-26">$16.36M</Drill><p>38.6% group margin</p></article>
        </div>
      </div>
      <div className="data-note">Source basis: TF+FA Consolidated Budget Forecast plus Tech Consolidated Budget Forecast. Values are AUD; charts use rounded $M figures.</div>
    </section>

    <section id="engines" className="chapter engines-section">
      <div className="chapter-label"><span>03</span><p>Growth engines</p></div>
      <div className="section-intro"><h2>Three businesses.<br/><em>One compounding system.</em></h2><p>Scale Engine builds delivery capacity. Growth Formula expands markets and talent. Margin Magic turns trusted relationships into recurring economics.</p></div>
      <div className="engine-grid">
        <article className="engine fa"><div className="engine-no">A</div><div><span>Frontline Accounting</span><h3>Scale Engine</h3><p>Domain-specialist managed services with an established delivery base and a digital margin layer.</p></div><dl><div><dt>FY26/27 revenue</dt><dd><Drill id="fa-engine">$25.7M</Drill></dd></div><div><dt>Headcount</dt><dd>761</dd></div><div><dt>FY28/29 EBITDA margin</dt><dd>28.4%</dd></div></dl></article>
        <article className="engine tf"><div className="engine-no">B</div><div><span>Talent Formula</span><h3>Growth Formula</h3><p>A multi-market recruitment capability spanning Australia, the UK and emerging US opportunity.</p></div><dl><div><dt>FY26/27 revenue</dt><dd><Drill id="tf-engine">$14.2M</Drill></dd></div><div><dt>YoY growth</dt><dd>+47.9%</dd></div><div><dt>FY28/29 revenue</dt><dd>$29.3M</dd></div></dl></article>
        <article className="engine ai"><div className="engine-no">C</div><div><span>AI Digital</span><h3>Margin Magic</h3><p>Recurring platform revenue distributed through trusted relationships across the combined client base.</p></div><dl><div><dt>FY26/27 revenue</dt><dd><Drill id="ai-engine">$2.13M</Drill></dd></div><div><dt>Gross margin</dt><dd>54.6%</dd></div><div><dt>EBITDA margin</dt><dd>27.6%</dd></div></dl></article>
      </div>
    ...
    {growthOpen && <div className="growth-layer" role="dialog" aria-modal="true" aria-labelledby="growth-title">
      <div className="growth-shell">
        ...
        <div className="growth-body">
          ...
          </div><div className="difference-bottom"><div><small>What the Board is underwriting</small><p>A team that has already built the delivery capability, client relationships and AI infrastructure needed to execute the plan.</p></div><div><small>What the Board is not underwriting</small><p>A concept-stage start-up, a financial-engineering story or growth that depends on a single business unit.</p></div><div className="result"><small>The intended result</small><strong>$34.1M → $72.8M</strong><p>with EBITDA margin moving from 9.1% to 27.8%.</p></div></div><div className="difference-source">Excel reference: TF+FA Consolidated + Tech Consolidated</div></section>}
          ...
          {growthTab==="priorities" && <section className="growth-priorities"><div className="growth-title"><small>FY2027 priorities</small><h3>Turn the thesis into<br/><em>sequenced execution.</em></h3><p>Growth depends on a small number of linked moves. Expand each priority to see why it matters, the evidence behind it and the proof point the Board should require.</p></div><div className="priority-evidence-list">
            {[
              ...
              ["Expand EBITDA","Move from 9.1% in FY25/26 to 14.3% in FY26/27 and 27.8% by FY28/29.","Pre/post-synergy bridge","Revenue growth alone does not validate the model. Margin expansion proves FA operating leverage, AI economics and integration benefits are compounding as intended.","TF+FA Consolidated rows 21 and 123–126 · Tech Consolidated rows 19 and 46","McKinsey research on large deals highlights persistent synergy governance and financial transparency as core conditions for converting strategic ambition into measurable value.","McKinsey · Keys to success in a large-deal merger","https://www.mckinsey.com/capabilities/m-and-a/our-insights/post-close-excellence-in-large-deal-m-and-a"],
